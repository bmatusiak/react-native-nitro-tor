...
namespace margelo::nitro::nitrotor {
  ...
  /**
   * A struct which can be represented as a JavaScript object (HttpPostParams).
   */
  ...
  public:
    HttpPostParams() = default;
    explicit HttpPostParams(std::string url, std::string body, std::string headers, double timeout_ms): url(url), body(body), headers(headers), timeout_ms(timeout_ms) {}
  };
